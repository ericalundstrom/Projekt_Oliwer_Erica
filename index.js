...
async function CreateBubbles(key) {
  ...
  let svg = d3.select("body").select("svg"); // Use existing SVG if available
  ...
  if (svg.empty()) {
    ...
    // Initialize the zoom before creating the circles
    let view;
    ...
    gViz
      ...
      .on("mousemove", function (event, d) {
        ...
        tooltip
          .html(`City: ${d.data.City}<br>${key}: ${d.data[key]}`)
    ...
    async function zoomTo(v, d) {
      ...
      gViz.attr(
        "transform",
        (d) =>
          `translate(${(d.x - view[0]) * k + 500},${(d.y - view[1]) * k + 400})`
      );
